...
export const chatService = {
  ...
  async getMessagesByConversationId(conversationId: string, userId: string) {
    ...
    if (![conversation.buyerId, conversation.sellerId].includes(userId)) {
      throw new AppError(
        "O usuário não pertence a esta conversa.",
  ...
  async createConversation(data: CreateConversationDto) {
    if (!data.productId || !data.buyerId || !data.sellerId) {
      throw new AppError(
        "Produto, comprador e vendedor são obrigatórios.",
        HttpStatus.BAD_REQUEST,
      );
    }

    const existingConversation =
      await chatRepository.findConversationByProductAndUsers(data);
  ...
  async sendMessage(data: CreateMessageDto) {
    if (
      ...
      throw new AppError(
        "Conversa, remetente, destinatário e conteúdo são obrigatórios.",
        HttpStatus.BAD_REQUEST,
      );
    }
